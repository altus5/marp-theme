...
// --- .marprc.yml 自動生成 ---
const MARPRC_PATH = path.resolve(".marprc.yml");

function ensureMarprc() {
  if (existsSync(MARPRC_PATH)) return;

  // .marprc.yml がない場合、node_modules/marp-theme/themes/ を参照する設定を自動生成
  const themesDir = path.join(PROJECT_ROOT, "themes");
  const relThemesDir = path.relative(process.cwd(), themesDir).replace(/\\/g, "/");

  const cssFiles = readdirSync(themesDir)
    .filter((f) => f.endsWith(".css") && !f.includes(".tailwind.") && !f.includes(".entry."))
    .sort();

  if (cssFiles.length === 0) return;

  const themeSet = cssFiles.map((f) => `  - ${relThemesDir}/${f}`).join("\n");
  const content = `# auto-generated by marp-build (add to .gitignore)\nthemeSet:\n${themeSet}\n`;

  writeFileSync(MARPRC_PATH, content, "utf8");
  console.log(`[marp-build] .marprc.yml を自動生成しました: ${MARPRC_PATH}`);
}
