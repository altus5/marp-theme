...
- [付録 スタイリングサンプル](#付録-スタイリングサンプル)
  - [A.1. アラートボックス](#a1-アラートボックス)
  - [A.2. インラインカラー](#a2-インラインカラー)
...
<!-- _header: "<a href='#はじめに'>はじめに</a><a href='#11-本書の目的'>1.1. 本書の目的</a>" -->
...
<!-- _header: "<a href='#はじめに'>はじめに</a><a href='#13-用語定義'>1.3. 用語定義</a>" -->
...
<!-- _header: "<a href='#第2章-システム概要'>第2章 システム概要</a><a href='#21-システム構成'>2.1. システム構成</a>" -->
...
<!-- _header: "<a href='#第2章-システム概要'>第2章 システム概要</a><a href='#22-開発スケジュール'>2.2. 開発スケジュール</a>" -->
...
結合テストの完了判定基準は、テスト項目の消化率100%かつ **重大障害（Severity A・B）** の残存ゼロとする。受入テストは発注者側の主導で実施し、受入確認書をもって本フェーズの完了とする。
...
スケジュールは暫定であり、要件定義フェーズの結果を踏まえて見直す可能性がある。変更が生じた場合は、プロジェクト管理計画書を更新のうえ、関係者に周知する。

</div>

<div class="column" data-label="Column">

**クラウド移行の検討**

本システムは現時点ではオンプレミス環境での稼働を前提としているが、将来的なクラウド移行を見据え、特定のインフラストラクチャに強く依存しない設計方針を採用している。データベース接続やファイルストレージのアクセスは抽象化レイヤーを介して行い、移行時の影響範囲を最小化する。

</div>

<div class="column-plain">

遅延リスクが顕在化した場合は、詳細設計・実装フェーズ内でのスコープ調整を優先し、リリース時期の変更は最終手段とする。

</div>

<div class="banner" data-label="Review">

**✔ レビューチェック 2.2**

</div>

各フェーズの開始前に、前フェーズの成果物レビューが完了していることを確認すること。特に要件定義書および基本設計書については、発注者・受注者双方の承認を得たうえで次フェーズに移行する。
...
<!-- _header: "<a href='#第3章-外部設計'>第3章 外部設計</a><a href='#31-顧客データ入力画面'>3.1. 顧客データ入力画面</a>" -->
...
<!-- _header: "<a href='#第3章-外部設計'>第3章 外部設計</a><a href='#31-顧客データ入力画面'>3.1. 顧客データ入力画面</a><span>入力項目一覧</span>" -->
...
<!-- _header: "<a href='#第3章-外部設計'>第3章 外部設計</a><a href='#31-顧客データ入力画面'>3.1. 顧客データ入力画面</a><span>バリデーション仕様</span>" -->
...
**(1) 企業名カナ**
全角カタカナ（ア〜ン）、長音符（ー）、**全角スペースのみ** 許可する。
正規表現: `^[ァ-ヶー　]+$`
...
<!-- _header: "<a href='#第3章-外部設計'>第3章 外部設計</a><a href='#31-顧客データ入力画面'>3.1. 顧客データ入力画面</a><span>処理仕様</span>" -->
...
<!-- _header: "<a href='#第3章-外部設計'>第3章 外部設計</a><a href='#31-顧客データ入力画面'>3.1. 顧客データ入力画面</a><span>エラーメッセージ一覧</span>" -->
...
<!-- _header: "<a href='#第3章-外部設計'>第3章 外部設計</a><a href='#32-社内データ更新画面'>3.2. 社内データ更新画面</a>" -->
...
<!-- _header: "<a href='#第3章-外部設計'>第3章 外部設計</a><a href='#32-社内データ更新画面'>3.2. 社内データ更新画面</a><span>業務ルール</span>" -->
...
<!-- _header: "<a href='#第3章-外部設計'>第3章 外部設計</a><a href='#32-社内データ更新画面'>3.2. 社内データ更新画面</a><span>レスポンス形式</span>" -->
...
<!-- _header: "<a href='#第3章-外部設計'>第3章 外部設計</a><a href='#33-セキュリティ情報確認画面'>3.3. セキュリティ情報確認画面</a>" -->
...
<!-- _header: "<a href='#第3章-外部設計'>第3章 外部設計</a><a href='#33-セキュリティ情報確認画面'>3.3. セキュリティ情報確認画面</a><span>セキュリティポリシー</span>" -->
...
<!-- _header: "<a href='#第4章-非機能要件'>第4章 非機能要件</a><a href='#41-性能要件'>4.1. 性能要件</a>" -->
...
<!-- _header: "<a href='#第4章-非機能要件'>第4章 非機能要件</a><a href='#43-セキュリティ要件'>4.3. セキュリティ要件</a>" -->
...
<!-- _header: "<a href='#第4章-非機能要件'>第4章 非機能要件</a><a href='#44-運用保守要件'>4.4. 運用・保守要件</a>" -->
...
本テーマはTailwind CSS v4のユーティリティクラスをビルド済みCSSに含んでおり、Markdown内のHTML要素に直接クラスを指定することで、テーマ定義クラスと組み合わせた柔軟なスタイリングが可能である。以下にその使用例を示す。

---

<!-- _header: "<a href='#付録-スタイリングサンプル'>付録 スタイリングサンプル</a><a href='#a1-アラートボックス'>A.1. アラートボックス</a>" -->

## A.1. アラートボックス

テーマCSSに定義された `.alert-*` クラスを使用する。`danger` / `warning` / `info` / `success` の4色バリエーションがあり、左ボーダーと背景色で視覚的に区別する。

<div class="alert-danger">

**[!] 重要（Critical）** — 本番環境への適用前に、必ずステージング環境での検証を完了すること。未検証のまま適用した場合、サービス停止のリスクがある。

</div>

<div class="alert-warning">

**[!] 注意（Warning）** — バッチ処理の実行中にマスタデータを更新した場合、処理結果に不整合が発生する可能性がある。更新作業はバッチ処理の完了後に行うこと。

</div>

<div class="alert-info">

**[i] 情報（Info）** — 本機能は2026年4月リリース予定のv2.0で追加される。現行バージョンでは利用できない。

</div>

<div class="alert-success">

**[OK] 確認済（Verified）** — 本セクションの記載内容は、2026年2月15日のレビュー会議にて発注者・受注者双方の承認を得ている。

</div>

> Marpのtwemojiプラグインが絵文字を \<img\> タグに変換し、\<strong\> 内の \<img\> がボックスの高さ計算を壊してしまうので、
> アラートボックスを使う場合は、絵文字を使わないようにしてください。

---

<!-- _header: "<a href='#付録-スタイリングサンプル'>付録 スタイリングサンプル</a><a href='#a2-インラインカラー'>A.2. インラインカラー</a>" -->

## A.2. インラインカラー

Tailwindユーティリティクラスを `<span>` 要素に直接指定し、文中の一部にカラーやバッジスタイルを適用する例を示す。

ステータスの表現例:

- 処理結果が <span class="text-green-800 font-bold">正常完了</span> の場合は、次のステップに進む
- 処理結果が <span class="text-red-800 font-bold">異常終了</span> の場合は、エラーログを確認する
- 処理結果が <span class="text-orange-800 font-bold">警告あり</span> の場合は、内容を精査のうえ判断する

色付き背景ラベル:

<span class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-bold">設計中</span>　<span class="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-bold">レビュー済</span>　<span class="bg-orange-100 text-orange-800 px-3 py-1 rounded-full text-sm font-bold">要確認</span>　<span class="bg-red-100 text-red-800 px-3 py-1 rounded-full text-sm font-bold">未着手</span>

Tailwindの `border` / `rounded-lg` / `p-4` / `flex` 等を組み合わせたカードレイアウト:
...
<span class="bg-green-100 text-green-600 px-3 py-1 rounded text-sm font-bold">全件合格</span>

</div>

テスト項目 48件中 48件が合格基準を満たしている。重大障害（Severity A・B）の残存はゼロであり、結合テストの完了判定基準を充足する。
